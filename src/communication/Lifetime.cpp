...
void Lifetime::mark_active() {
    activated_at = WSTime::get_epoch_ms();
}

void Lifetime::mark_inactive() {
    deactivated_at = WSTime::get_epoch_ms();
}

bool Lifetime::is_timeout(t_time_epoch_ms now) const {
    if (activated_at == 0 || deactivated_at > activated_at) {
        // アクティブ状態
        ...
        VOUT(now);
        VOUT(is_mortal);
        VOUT(is_actually_timeout);
        VOUT(lifetime_inactive);
...
const t_time_epoch_ms Lifetime::LIFETIME_ROUNDTRIP_INACTIVE = 1 * SECOND_IN_MS;
