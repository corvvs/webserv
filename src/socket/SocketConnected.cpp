...
SocketConnected::SocketConnected(const SocketConnected &other)
    : ASocket(other) {}

SocketConnected::SocketConnected(t_socket_domain sdomain, t_socket_type stype)
    : ASocket(sdomain, stype) {}

SocketConnected::SocketConnected(t_fd accepted_fd,
                                 SocketListening &listening_socket)
    : ASocket(accepted_fd, listening_socket.get_domain(),
...
SocketConnected *SocketConnected::connect(t_socket_domain sdomain,
...
int SocketConnected::shutdown() { return ::shutdown(get_fd(), SHUT_RDWR); }

int SocketConnected::shutdown_write() { return ::shutdown(get_fd(), SHUT_WR); }

int SocketConnected::shutdown_read() { return ::shutdown(get_fd(), SHUT_RD); }