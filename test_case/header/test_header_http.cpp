...
TEST(header_http, spaces_ok) {
    const char *strs[] = {"Host: aaa", "Host: ", "Host:", " Host: aaa", "Host: 123123", NULL};
...
TEST(header_http, value_ok) {
    const char *strs[] = {"Host: aiueo", "Host:aiueo", "Host:aiueo    ", "host: aiueo", NULL};
    for (size_t i = 0; strs[i]; ++i) {
...
TEST(header_http, value_ok_2) {
    const char *strs[] = {"Host:a i u e o", "Host: a i u e o", "Host:a i u e o    ", NULL};
    for (size_t i = 0; strs[i]; ++i) {
        const HTTP::byte_string s = HTTP::strfy(strs[i]);
        const HTTP::light_string l(s);
        HeaderHolderHTTP holder;
...
TEST(header_http, duplication) {
    const char *strs[] = {"Host:aiueo", "Host: 1234", "Host: xyz", " hoST:beepbeep", NULL};
    HeaderHolderHTTP holder;
    for (size_t i = 0; strs[i]; ++i) {
        const HTTP::byte_string s = HTTP::strfy(strs[i]);
        const HTTP::light_string l(s);
